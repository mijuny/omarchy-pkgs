...
/* Speaker mute LED - found via DSDT analysis on ASUS ExpertBook B9406CAA.
 * Mirrors ASUS_WMI_DEVID_MICMUTE_LED structure (adjacent GPIO 0x001A0885). */
#define ASUS_WMI_DEVID_AUDIO_MUTE_LED 0x0004001C
...
struct asus_wmi {
	...
	struct led_classdev audio_mute_led;
...
static int audio_mute_led_set(struct led_classdev *led_cdev,
			      enum led_brightness brightness)
{
	int state = brightness != LED_OFF;
	int err;

	err = asus_wmi_set_devstate(ASUS_WMI_DEVID_AUDIO_MUTE_LED, state, NULL);
	return err < 0 ? err : 0;
}
...
static void asus_wmi_led_exit(struct asus_wmi *asus)
{
	...
	led_classdev_unregister(&asus->audio_mute_led);
...
static int asus_wmi_led_init(struct asus_wmi *asus)
{
	...
	if (asus_wmi_dev_is_present(asus, ASUS_WMI_DEVID_AUDIO_MUTE_LED)) {
		asus->audio_mute_led.name = "platform::mute";
		asus->audio_mute_led.max_brightness = 1;
		asus->audio_mute_led.brightness_set_blocking = audio_mute_led_set;
		asus->audio_mute_led.default_trigger = "audio-mute";
